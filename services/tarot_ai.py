...
async def gpt_make_prediction(
    ...
    prompt = f"""
Ты — опытный таролог. Русский язык, чётко и по делу. Один компактный ответ без вступлений.
...
Оформление:
- НИКАКОГО маркдауна/буллетов: *, -, •, —, >, заголовков, жирного/курсива.
- Для строк с картами используй маркер "⭐️ " (звезда и пробел), затем «Название — толкование».
- Каждый абзац начинай символом "⭐️ ".
- Итог выводи отдельным абзацем и начинай строку с "🌙 Итог:" (без звезды).

Формат:
⭐️ Название карты — краткое толкование строго в контексте темы{(' и уточнения' if scenario_ctx else '')}.
⭐️ Название карты — краткое толкование ...
⭐️ Название карты — краткое толкование ...

🌙 Итог: 2–4 предложения резюме смысла карт БЕЗ советов/рекомендаций/инструкций.
Запрещены побудительные формулировки («нужно», «стоит», «следует», «совет», «сделайте», «попробуйте» и т.п.).
""".strip()
    ...
    txt = _ensure_moon_on_itog(txt)                       # нормализуем Итог -> 🌙 Итог:
    txt = _prefix_paragraphs_with_stars_except_itog(txt)  # ставим ⭐️ везде, кроме строки Итога
    return txt
