...
Формат:
1) Сначала одной строкой перечисли карты Совета: "Карты: <карта1>, <карта2>, ...".
Далее отступить одну строку.
...
async def gpt_make_advice_from_yandex_answer(
    *,
    ...
    Для 1 карты — краткий сухой совет, для 3 карт — подробный и длинный.
    В начале всегда выводим строку: 'Карты: ...'
    """
    # Явно формируем строку с картами
    if advice_cards_list:
        cards_line_for_prompt = f"Карты: {', '.join(advice_cards_list)}\n"
    else:
        cards_line_for_prompt = "Карты: (если нет — пропусти эту строку)\n"

    # Настройки длины совета
    if advice_count == 3:
        guidance = "Сделай связный, практичный и развёрнутый совет на 10–15 предложений."
    else:
        guidance = "Сделай краткий, практичный совет на 1 предложение, сухой ответ."
